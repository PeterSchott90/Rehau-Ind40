...
public class FauRehauContext {
	...
	public static void addQuestion(String question) {
		for(FauRehauGroup frGroup : groupCollection) {
			frGroup.addQuestion(question);
		}
	}

	public static void setQuestionAnswered(int groupId, int userId, int points) {
		FauRehauGroup frGroup = getGroupById(groupId);
		frGroup.setQuestionAnswered(userId, points);
	}
